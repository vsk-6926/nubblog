...
const SinglePost = () => {
  ...
  const path = location.pathname.split("https://nubblog.herokuapp.com/")[2];
  ...
  const handleClick = async (e) => {
    try {
      await axios.delete("https://nubblog.herokuapp.com/posts/" + path, {
        data: { username: user.username },
      });
      window.location.replace("https://nubblog.herokuapp.com/");
    } catch (err) {}
  };

  const handleUpdate = async (e) => {
    try {
      await axios.put("https://nubblog.herokuapp.com/posts/" + path, {
  ...
  useEffect(() => {
    const getPost = async () => {
      const res = await axios.get("https://nubblog.herokuapp.com/posts/" + path);
      setPost(res.data);
